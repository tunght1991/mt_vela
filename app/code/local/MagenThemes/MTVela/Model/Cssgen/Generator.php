...
?>
<?php
class MagenThemes_MTVela_Model_Cssgen_Generator extends Mage_Core_Model_Abstract{
    ...
    protected function _generateStoreCss($design, $storeCode){
        ...
        $filedefault = Mage::helper('mtvela/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'magenthemes/mtvela/css/' . $design . '.phtml';
        ...
        try{
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('mtvela')->__("Template file is empty or doesn't exist: %s", $path) );
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array( 'path' => Mage::helper('mtvela/cssgen')->getGeneratedCssDir() ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mtvela')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('mtvela/cssgen')->getGeneratedCssDir()). '<br/>Message: ' . $gener->getMessage());
